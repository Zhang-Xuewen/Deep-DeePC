...
class control(object):
    ...
    def _data_process(self, args):
        ...
        for i in range(len(self.control_sp)):
            ...
            us_all.append(np.tile(us, (self.N if i != 0 else self.N + self.Tini, 1)))
            ys_all.append(np.tile(ys, (self.N if i != 0 else self.N + self.Tini, 1)))
            us_all_.append(np.tile(us_, (self.N if i != 0 else self.N + self.Tini, 1)))
            ys_all_.append(np.tile(ys_, (self.N if i != 0 else self.N + self.Tini, 1)))
